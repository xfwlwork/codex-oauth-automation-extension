...
- 支持 HeroSMS 接码平台自动完成手机号验证：Step 8 遇到 `/add-phone` 页面时，自动获取手机号、填写、轮询验证码并提交，完成 OAuth 授权
...
- 至少准备一种验证码接收方式：
  ...
  - 手动填写一个可收信邮箱
- 如果遇到手机号验证页面（`/add-phone`），可选配置 HeroSMS 接码平台自动完成验证
...
用于自动完成 OpenAI 手机号验证（`/add-phone` 页面）。

- `接码平台`：选择 `HeroSMS` 或 `不使用`
- `API Key`：HeroSMS 接码平台的 API Key
- `国家`：选择接码国家，默认泰国（`52`）

配置后，Step 8 遇到手机号验证页面时会自动：
1. 从 HeroSMS 获取一个虚拟手机号
2. 填写到认证页面并提交
3. 等待跳转到验证码页面
4. 轮询 HeroSMS 获取短信验证码
5. 填写并提交验证码，完成手机号验证

未配置时，遇到手机号页面会直接报错，需要手动处理。
