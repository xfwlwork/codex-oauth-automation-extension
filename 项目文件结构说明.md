...
- `background/sms-api.js`：HeroSMS 接码平台 API 客户端，兼容 SMS-Activate 协议，负责获取手机号、轮询验证码、激活与取消操作。
- `background/sms-phone-flow.js`：SMS 手机号验证子流程编排器，负责从 HeroSMS 获取号码、填写手机号、等待跳转、轮询验证码、填写并提交验证码的完整链路。
...
- `content/signup-page.js`：注册/登录/授权主内容脚本，负责 OpenAI / ChatGPT 页面上的步骤执行，以及手机号验证页面的号码与验证码填写。
...
- `tests/sms-api.test.js`：测试 HeroSMS API 响应解析与错误码识别，覆盖纯函数解析逻辑。
